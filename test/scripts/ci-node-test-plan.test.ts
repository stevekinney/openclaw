...
describe("scripts/lib/ci-node-test-plan.mts", () => {
      ...
      for (const shard of dense.filter((entry) => entry.groups.length > 10)) {
        expect(shard).toMatchObject({
          planConcurrency:
            ...
            shard.groups.some((group) => group.configs.some(isExclusiveCiTestConfig))
              ? 1
              : 2,
          requiresDist: false,
          runner: EXTRA_LARGE_NODE_TEST_RUNNER,
  ...
  it("preserves runtime preparation and core-only ownership in full and compact plans", () => {
    ...
    for (const { group, job } of placements) {
      ...
      // Autosizing a proven sibling keeps this runtime consumer's old ceiling.
      expect(group.env).toEqual(
        job.planConcurrency === 1 && job.groups.some((entry) => entry.fallbackMaxWorkers === 2)
